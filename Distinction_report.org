...
We understand that for some larger forkAfter values the result should be faster than the sequential solver however with a lower forkAfter value it is going to take longer due to the overhead when joining together all the subtasks making it slower than the sequential solve.The results can also differ if a better computer was used, with more cores and threads.
