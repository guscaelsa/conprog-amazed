...
from the tests it seemed clear that the seuential solution was faster then most of the parallel searches depending on the 
forkAfter value. (add valid testresult here) We understand that for some larger forkAfter values the result should be faster than the sequential solver however with a lower forkAfter value it is going to take longer ddue to the overhaed when joining together all the subtasks making it slower than the sequential solve.
...
cells the player could move to above the lerga map.